...
token = (
    ("0x" + topics[1][26:66]) if topics[1] != wbnb_topic else ("0x" + topics[2][26:66])
)
print(token)
# %%
token
# %%
token_tx_list = scan("account", "tokentx", contractaddress=token, address=lp_addr)
bnb_tx_list = scan("account", "tokentx", contractaddress=wbnb_addr, address=lp_addr)
# %%
...
for index, event in logs.iterrows():
    try:
        ...
        token = (
            "0x" + event["topics"][1][26:66]
            if event["topics"][0] != wbnb_topic
            else "0x" + event["topic"][2][26:66]
        )

        token_tx_list = scan(
            "account", "tokentx", contractaddress=token, address=lp_addr
        )
        bnb_tx_list = scan(
            "account", "tokentx", contractaddress=wbnb_addr, address=lp_addr
        )
...
# %%
lp_contract = w3.eth.contract(address=w3.toChecksumAddress(lp_addr), abi=pancake_lp_abi)
# %%
...
def filter_add_liquidity(tx, token: str) -> bool:
    try:
        ...
        return (
            input[0].abi["name"] == "addLiquidityETH"
            and input[1]["token"].lower() == token.lower()
        )
    except:
        return False
...
def new_token_stat(pair_created_event) -> Stat:
    ...
    token = (
        pair_created_event["topics"][2]
        if pair_created_event["topic"][1] == wbnb_topic
        else pair_created_event["topics"][1]
    )
    create_pair_tx = w3.eth.get_transaction(pair_created_event["transactionHash"])
    owner = create_pair_tx["from"]

    owner_tx = scan(
        "account", "txlist", address=owner, startbock=create_pair_tx["blockNumber"]
    )

    add_liquidity_bnb = int(
        list(filter(lambda x: filter_add_liquidity(x, token), owner_tx))[0]["value"]
    )
