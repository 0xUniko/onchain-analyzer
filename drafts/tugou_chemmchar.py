...
lp_contract = w3.eth.contract(address=w3.toChecksumAddress(lp_addr), abi=lp_abi)
...
# %%
token_tx_list = scan("account", "tokentx", contractaddress=cheemchar, address=lp_addr)
...
# %%
list(filter(lambda x: x["hash"] == lp_logs[-5]["transactionHash"], token_tx_list))

# %%
len(bnb_tx_list)
# %%
list(filter(lambda x: x["hash"] == lp_logs[-5]["transactionHash"], bnb_tx_list))
...
# %%
len(list(filter(lambda x: decode_pair_event(x["topics"][0]) == "swap", lp_logs)))
# %%
