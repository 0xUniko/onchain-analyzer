...
token_transfer_logs = token_logs.loc[
    token_logs["topic0"].map(hex_to_topic_name) == "transfer"
]
token_approve_logs = token_logs.loc[
    token_logs["topic0"].map(hex_to_topic_name) == "approve"
]
...
def check_approve(addr):
    ...
    if addr_approve_topic2.nunique() == 0:
        return True
    if (
        addr_approve_topic2.nunique() == 1
        and addr_approve_topic2.values[0] == pancake_router_topic
    ):
        return True
    return False
...
def decode_router_input(tx):
    ...
    if "swap" in method_name:
        ...
        amount0In, amount1In, amount0Out, amount1Out = (
            int(swap_data[0][2 + 64 * i : 2 + 64 * (i + 1)], 16) for i in range(4)
        )
...
def print_balance(addr):
    ...
    for i, (_, tx) in enumerate(addr_txs.iterrows()):
        print(
            i,
            datetime.date.fromtimestamp(int(tx["timeStamp"])),
    ...
    for _, log in addr_transfer_logs.loc[
        addr_transfer_logs["transactionHash"].isin(hashs)
    ].iterrows():
        value = (
            ("0x" + log["topic1"][26:66], int(log["data"], 16) / 10**18)
            if log["topic2"] == addr_topic
            else ("0x" + log["topic2"][26:66], -int(log["data"], 16) / 10**18)
        )
        # bbb.append(value)
...
def print_entity_balance(entity):
    print("check_approve:", pd.Series([check_approve(addr) for addr in entity]).all())
...
def get_entity_balance(entity, transfer_logs):
    if len(entity) == 1:
        return {
            "entity": entity,
            "token_in": transfer_logs.loc[
                transfer_logs["topic2"].map(lambda x: "0x" + x[26:66]).isin(entity),
                "data",
            ]
            .map(lambda x: int(x, 16))
            .sum(),
            "token_out": transfer_logs.loc[
                transfer_logs["topic1"].map(lambda x: "0x" + x[26:66]).isin(entity),
                "data",
            ]
            .map(lambda x: int(x, 16))
            .sum(),
...
def get_all_entities_balance(transfer_logs, historical_holders):
    ...
    entities_balance["token_amount"] = (
        entities_balance["token_in"] - entities_balance["token_out"]
    )
...
def get_entities_balance_daily(transfer_logs, historical_holders):
    ...
    dates = sort(
        transfer_logs["timeStamp"].map(lambda x: datetime.date.fromtimestamp(x))
    )
    start_date = dates[0]
    end_date = dates[-1]
    daily_logs = {}
...
# %%
entities_balance = get_all_entities_balance(token_transfer_logs, historical_holders)
# %%
entities_balance
# %%
(
    entities_balance_in,
    entities_balance_out,
    entities_balance_value,
) = get_entities_balance_daily(token_transfer_logs, historical_holders)
# %%
pd.set_option("display.max_columns", None)
entities_balance_value.sort_values(str(token_start_date), ascending=False).applymap(
    lambda x: x / 10**18 if type(x) == int else x
).head(50)
# %%
entities_balance_accum = entities_balance_value.apply(lambda x: x[1:].cumsum(), axis=1)
# %%
# should be false
...
swap_records = (
    ...
    .loc[
        ...
        & (
            pair_logs[0]["timeStamp"].map(lambda x: datetime.date.fromtimestamp(x))
            == token_start_date
        )
    ]
    .apply(
        lambda x: {
            **decode_swap_event(x["topic1"], x["topic2"], x["data"]),
