...
create_mvs_events = [
    log
    for _, log in pairCreated_logs.iterrows()
...
def is_add_liquidity(input):
    method_name = pancake_router_contract.decode_function_input(input)[0].abi["name"]
...
while not date > today:
    ...
    swap_cnt[str(date)] = len(
        mvs_transfer_logs[
            mvs_transfer_logs["timeStamp"].map(
                lambda x: start_timestamp_of_date <= int(x) <= end_timestamp_of_date
            )
        ]
    )

    logs_cnt[str(date)] = len(
        mvs_logs[
            mvs_logs["timeStamp"].map(
                lambda x: start_timestamp_of_date <= int(x) <= end_timestamp_of_date
            )
        ]
    )
...
hashs.extend(
    mvs_logs.loc[
        mvs_logs["timeStamp"].map(
            lambda x: str(datetime.date.fromtimestamp(x)) == "2021-12-30"
        ),
        "transactionHash",
    ]
)
...
with TxsGetter() as txs_getter:
    txs = txs_getter.get_router_swap_txs_by_token(
        mvs, datetime.date(2021, 12, 30), datetime.date(2021, 12, 30)
    )
# %%
...
pair_tokens = [
    busd_addr.lower(),
    bsc_usd_addr.lower(),
    "0xba2ae424d960c26247dd6c32edc70b295c744c43".lower(),
    wbnb_addr.lower(),
]
pair_logs_dict2 = {}
...
# %%
mvs_transfer_logs = mvs_logs.loc[
    mvs_logs["topic0"].map(hex_to_topic_name) == "transfer"
]
mvs_approve_logs = mvs_logs.loc[mvs_logs["topic0"].map(hex_to_topic_name) == "approve"]
# %%
mvs_logs["topic2"].value_counts()

# %%
pd.concat(
    [mvs_transfer_logs["topic1"], mvs_transfer_logs["topic2"]]
...
addr_topic = (
    transfer_cnt.loc[
        transfer_cnt > 5 & transfer_cnt.index.to_series().isin(historical_holders)
    ]
    .index.to_series()
    .sample(1)
    .iloc[0]
...
print(addr)
# %%
mvs_approve_logs.loc[mvs_approve_logs["topic1"] == addr_topic, "topic2"].value_counts()
# %%
# only useful thing in approval should be the spender
mvs_approve_logs.loc[mvs_approve_logs["topic1"] == addr_topic, "transactionHash"].iloc[
    0
]
# %%
...
# %%
addr_approve_logs = mvs_approve_logs.loc[
    mvs_approve_logs["topic1"] == addr_topic, "topic2"
]
if addr_approve_logs.empty:
    pass
elif (
    addr_approve_logs.iloc[0] == pancake_router_topic
    and addr_approve_logs.nunique() == 1
):
    addr_transfer_logs = mvs_transfer_logs.loc[
        (mvs_transfer_logs["topic1"] == addr_topic)
        | (mvs_transfer_logs["topic2"] == addr_topic),
...
# %%
if addr_txs.index.to_series().isin(addr_transfer_logs["transactionHash"]).all():
...
for i in range(4):
    pair_logs_hash_set = pair_logs_hash_set.union(pair_logs[i]["transactionHash"])
# %%
