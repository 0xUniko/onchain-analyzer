from utils.nft_transactions_analysis import weth
...
looksrare_addr = cast(ChecksumAddress,
...
def get_taker_balance(account: HexAddress, taker: TakerEvent,
                      loyalty: LoyaltyPaymentDict | None,
                      taker_type: TakerType) -> DealBalance:
    assert taker['collection'] == loyalty['collection'] and taker[
        'tokenId'] == loyalty[
            'tokenId'] if loyalty is not None else True, 'Taker event and LoyaltyPayment event does not match'

    if (taker_type == TakerType.taker_bid and taker['maker'].lower() !=
            account.lower()) or (taker_type == TakerType.taker_ask
                                 and taker['maker'].lower() == account.lower):
        eth = -taker['amount'] / 10**18
    else:
        assert loyalty[
            'currency'] == weth if loyalty is not None else True, 'loyalty not paid in weth'

        eth = (taker['amount'] -
               loyalty['amount'] if loyalty is not None else 0) / 10**18
