...
busd_addr_created_log = {"token0": mvs, "token1": busd_addr, "pair": pair_addrs[0]}
...
def check_approve(addr):
    ...
    addr_approve_topic2 = mvs_approve_logs.loc[
    ...
    if check_approve(addr):
        ...
        for _, tx in addr_txs.iterrows():
            ...
            if "swap" in method_name:
                ...
                amount0In, amount1In, amount0Out, amount1Out = (
                    int(swap_data[0][2 + 64 * i : 2 + 64 * (i + 1)], 16)
                    for i in range(4)
                )
        ...
        for hash in set(addr_transfer_logs["transactionHash"]) - set(addr_txs.index):
            logs = addr_transfer_logs.loc[addr_transfer_logs["transactionHash"] == hash]
