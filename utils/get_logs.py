...
@retry(stop=stop_after_attempt(10),
       ...
       reraise=True)
def get_logs_by_block_interval(start_block: int, end_block: int, address: str,
                               scan, **topics_param):
    logs = []
    length = 1000
...
def get_logs(name: str, address: str, date: datetime.date, scan,
             **topics_param):
    ...
    if os.path.exists(filename):
        ...
        logs = get_logs_by_block_interval(start_block, end_block, address,
                                          scan, **topics_param)
        ...
        logs = pd.concat([logs_cache, pd.DataFrame(logs)], ignore_index=True)
    else:
...
def get_token_logs(token_name: str, token_addr: str, proxies: str = None):
    with Scanner(proxies=proxies) as scanner:
        start_timestamp_hex = scanner.scan('logs',
                                           'getLogs',
                                           address=token_addr)[0]['timeStamp']
        ...
        while not start_date > today:
            logs.append(
                get_logs(token_name, token_addr, start_date, scanner.scan))
...
def get_pair_logs(token_name: str,
                  token_addrs: List[str],
                  proxies: str = None):
    return [
        get_token_logs(os.path.join(token_name, f'pair{i}'), token, proxies)
        for i, token in enumerate(token_addrs)
    ]


def get_pairCreated_logs(start_date: datetime.date = None,
                         proxies: str = None):
    with Scanner(proxies=proxies) as scanner:
