...
EvInventory_event_sig = w3.keccak(
    text="EvInventory(bytes32,address,address,uint256,uint256,uint256,uint256,uint256,address,bytes,(uint256,bytes),(uint8,uint256,uint256,uint256,bytes32,address,bytes,uint256,uint256,uint256,(uint256,address)[]))"
...
class EvInventoryDict(TypedDict):
    ...
    detail: Tuple[
        ...
        int,
        int,
        List[Tuple[int, ChecksumAddress]],
    ]
...
class EvInventoryEvent:
    ...
    def __init__(self, ev_inventory_dict: EvInventoryDict):
        ...
        self.detail = {
            ...
            "orderIdx": detail[1],
            "itemIdx": detail[2],
            "price": detail[3] / 10**18,
            "itemHash": HexBytes(detail[4]),
            "executionDelegate": detail[5],
            "dataReplacement": detail[6],
            "bidIncentivePct": detail[7],
            "aucMinIncrementPct": detail[8],
            "aucIncDurationSecs": detail[9],
            "fee": [{"percentage": fee[0] / 10000, "to": fee[1]} for fee in detail[10]],
        }
...
def get_x2y2_tx_balance(
    ev_inventory: EvInventoryEvent, ev_profit: EvProfitDict, account: HexAddress
) -> DealBalance:
    assert (
        ev_inventory.detail["itemHash"] == ev_profit["itemHash"]
    ), "itemHash does not match"

    assert (
        ev_inventory.detail["op"] == Op.COMPLETE_SELL_OFFER
        or ev_inventory.detail["op"] == Op.COMPLETE_BUY_OFFER
    ), "not COMPLETE_SELL_OFFER or COMPLETE_BUY_OFFER"
