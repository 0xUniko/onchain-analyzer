...
from utils.token.pancake_utils import wbnb_topic, wbnb_addr
...
async def get_txs(index, event, length, scanner, semaphore):
    ...
    token = (
        ("0x" + event["topic1"][26:66])
        if event["topic0"] != wbnb_topic
        else ("0x" + event["topic2"][2][26:66])
    )

    try:
        for attemp in Retrying(
            stop=stop_after_attempt(100), wait=wait_random(min=1, max=1.5)
        ):
            with attemp:
                ...
                if (
                    len(token_tx_list) >= 500 * (length - index) / length + 50
                    or len(bnb_tx_list) >= 500 * (length - index) / length + 50
                ):
                    print(index, "    ", token)
                    return token, "token"
...
async def get_liquided_tokens(date: datetime.date):
    async with Scanner() as scanner:
        ...
        with open(
            os.getcwd() + "/utils/filtered_tokens/" + date.strftime("%Y%m%d") + ".json",
            "w",
        ) as f:
            f.write(
                json.dumps(
                    {
                        "tokens": list(filter(lambda x: x[1] == "token"), tokens),
                        "errors": list(filter(lambda x: x[1] == "error"), tokens),
                    }
                )
            )
