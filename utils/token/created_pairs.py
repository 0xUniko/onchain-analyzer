...
from utils.token.pancake_utils import (
    wbnb_addr,
    busd_addr,
    bsc_usd_addr,
    wbnb_topic,
...
add_liquidity_eth_inputs = liquidity_txs.loc[
    liquidity_txs["method_name"] == "addLiquidityETH"
].apply(
    lambda x: {"timeStamp": x["timeStamp"], **x["data"]}, axis=1, result_type="expand"
)

add_liquidity_busd_inputs = liquidity_txs.loc[
    liquidity_txs.apply(
        lambda x: x["method_name"] == "addLiquidity"
        and (x["data"]["tokenA"] == busd_addr or x["data"]["tokenB"] == busd_addr),
        axis=1,
    )
].apply(
    lambda x: {"timeStamp": x["timeStamp"], **x["data"]}, axis=1, result_type="expand"
...
def first_add_liquidity_log(token):
    eth_log = add_liquidity_eth_inputs.loc[
        add_liquidity_eth_inputs["token"] == token
    ].sort_values("deadline")
    busd_log = add_liquidity_busd_inputs.loc[
        (add_liquidity_busd_inputs["tokenA"] == token)
        | (add_liquidity_busd_inputs["tokenB"] == token)
    ].sort_values("deadline")
    ...
    if eth_log.empty:
        if busd_log.empty:
            ...
            if busd_log.iloc[0]["tokenB"] == token:
                return {
                    "busd": busd_log.iloc[0]["amountAMin"] / 10**18,
                    "bnb": 0,
                    "transactionHash": busd_log.iloc[0].name,
                ...
                return {
                    "busd": busd_log.iloc[0]["amountBMin"] / 10**18,
                    "bnb": 0,
                    "transactionHash": busd_log.iloc[0].name,
                    "date": datetime.date.fromtimestamp(
                        int(busd_log.iloc[0]["timeStamp"])
        ...
        if busd_log.empty:
            return {
                "bnb": eth_log.iloc[0]["amountETHMin"] / 10**18,
                "busd": 0,
                "transactionHash": eth_log.iloc[0].name,
                "date": datetime.date.fromtimestamp(int(eth_log.iloc[0]["timeStamp"])),
            }
        else:
...
# %%
created_tokens_date = (
    pairCreated_logs.loc[
        pairCreated_logs.apply(
            lambda x: (
                x["topic1"] == busd_topic
                or x["topic1"] == wbnb_topic
                or x["topic2"] == busd_topic
                or x["topic2"] == wbnb_topic
            )
            and datetime.date.fromtimestamp(x["timeStamp"])
            == datetime.date(2022, 1, 3),
            axis=1,
        )
    ]
    .apply(
        lambda x: "0x" + x["topic1"][26:66]
        if x["topic2"] == busd_topic or x["topic2"] == wbnb_topic
        else "0x" + x["topic2"][26:66],
        axis=1,
    )
    .value_counts()
    .index.to_series()
...
initial_liquidities = tokens_with_liquidity.to_frame().apply(
    lambda x: {"token": x[0], **first_add_liquidity_log(x[0])},
    axis=1,
    result_type="expand",
)
# %%
...
# %%
initial_liquidities_and_txs_amounts = (
    initial_liquidities.set_index("token")
    .join(pd.Series(txs_tokens1).value_counts().rename("txs_amounts"))
    .sort_values("txs_amounts", ascending=False)
)
# %%
...
"""
pd.concat([pairCreated_logs["topic1"], pairCreated_logs["topic2"]]).value_counts().head(
    20
)
# %%
...
busd_token = pairCreated_logs.loc[
    ...
    | (pairCreated_logs["topic2"] == busd_topic)
].apply(lambda x: x["topic2"] if x["topic1"] == busd_topic else x["topic1"], axis=1)

wbnb_token = pairCreated_logs.loc[
    (pairCreated_logs["topic1"] == wbnb_topic)
    | (pairCreated_logs["topic2"] == wbnb_topic)
].apply(lambda x: x["topic2"] if x["topic1"] == busd_topic else x["topic1"], axis=1)
...
bscusd_token = pairCreated_logs.loc[
    ...
    | (pairCreated_logs["topic2"] == bscusd_topic)
].apply(lambda x: x["topic2"] if x["topic1"] == busd_topic else x["topic1"], axis=1)

print(bscusd_token.isin(set(bscusd_token).union(set(wbnb_token))).value_counts())
# %%
